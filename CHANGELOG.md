...
- [core] Workaround root shell mount namespace issues on older Myfuck versions
...
- Introduce a new flag: `Shell.FLAG_USE_MYFUCK_BUSYBOX`. With this flag set, `/sbin/.magisk/busybox` will be prepended to `PATH`, so the shell will use Magisk's internal busybox.
